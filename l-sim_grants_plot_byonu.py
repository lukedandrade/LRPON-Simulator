    ...
    for param in parameters:
        try:
            for NUMBER_OF_ONUs in NUMBER_OF_ONUs_vet:
                IPACT_Pred_grant_medio[NUMBER_OF_ONUs] = {}
                ipact_basic_grant_medio[NUMBER_OF_ONUs] = {}
                IPACT_Pred_std[NUMBER_OF_ONUs] = {}
                ipact_basic_std[NUMBER_OF_ONUs] = {}
                IPACT_Pred_grant_percent_mean[NUMBER_OF_ONUs] = {}
                IPACT_Pred_grant_percent_std[NUMBER_OF_ONUs] = {}
            ...
            for size in PKT_SIZES:
                for exp in exponents:
                    for NUMBER_OF_ONUs in NUMBER_OF_ONUs_vet:
                        ...
                        IPACT_Pred_grant_medio[NUMBER_OF_ONUs][exp] = media
                        ...
                        IPACT_Pred_grant_percent_mean[NUMBER_OF_ONUs][exp] = percentage_media
                        IPACT_Pred_grant_percent_std[NUMBER_OF_ONUs][exp] = percentage_std
            ...
            for size in PKT_SIZES:
                for exp in exponents:
                    for NUMBER_OF_ONUs in NUMBER_OF_ONUs_vet:
                        ...
                        std_pkt_loss = []
                        for seed in seeds:
                            df_tmp = pd.read_csv("test_poissonPG_IPACT/{}km/{}/csv/grant_usage/ipact-dist{}-{}ONUs-{}OLTs-{}-exp{}-pkt{}-s{}-grant_usage.csv".format(DISTANCE, size, DISTANCE,
                                                                                                                                        NUMBER_OF_ONUs, NUMBER_OF_OLTs,
                        ...
                        t_test_results = calculateT_student(ipact_mean, 10)
                        std = t_test_results['t-test amp']
                        ipact_basic_grant_medio[NUMBER_OF_ONUs][exp] = media
            ...
            title = "All ONUs - {} OLT ({}km) - Trafego {}, {} - média de Largura de banda transmitida  5s de simulação ({}) w{}-p{}".format(NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, model, param['w'], param['p'])
            filename = "All_ONUs_{}OLT_{}km-{}-{}-grant_sizes-{}_w{}-p{}".format(NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, model, param['w'], param['p'])
            plt.title(title)
            plt.xlabel("load (%)")
            ...
            plt.clf()
            plt.figure(figsize=(14, 10))
            title = "All ONUs - {} OLT ({}km) - Trafego {}, {} - Grants preditos em 5s de simulação ({}) w{}-p{}".format(NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, model,  param['w'], param['p'])
            filename = "All_ONUs_{}OLT_{}km-{}-{}-n_grants-{}_w{}-p{}".format(NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, model, param['w'], param['p'])
            plt.title(title)
            plt.xlabel("load (%)")
            ...
            i=0
            for NUMBER_OF_ONUs in NUMBER_OF_ONUs_vet:
                aux_df = pd.DataFrame(ipact_df_percentages_media[NUMBER_OF_ONUs])
                aux_label = "Ipact pred, {}, load(MB) = {:.3f}, n_ONUs = {}".format(model, float(PKT_SIZES[0])/(1024**2), NUMBER_OF_ONUs)
                media_pl = np.array(aux_df.iloc[:,0])
                aux_df = pd.DataFrame(ipact_df_percentages_std[NUMBER_OF_ONUs])
