    ...
    for param in parameters:
        try:
            for model in PRED_ALG:
                IPACT_Pred_r2_medio[model] = {}
                IPACT_Pred_mse_medio[model] = {}
                IPACT_Pred_r2_std[model] = {}
                IPACT_Pred_mse_std[model] = {}
            ...
            for size in PKT_SIZES:
                for exp in exponents:
                    for model in PRED_ALG:
                        ...
                        IPACT_Pred_r2_medio[model][exp] = media
                        IPACT_Pred_r2_std[model][exp] = std
            ...
            for size in PKT_SIZES:
                for exp in exponents:
                    for model in PRED_ALG:
                        ...
                        t_test_results = calculateT_student(ipact_mean, 10)
                        std = t_test_results['t-test amp']
                        IPACT_Pred_mse_medio[model][exp] = media
                        IPACT_Pred_mse_std[model][exp] = std
            ...
            title = "{} ONUs - {} OLT ({}km) - Trafego {}, {} - média de R2 Score em 5s de simulação ({}) w{}-p{}".format(NUMBER_OF_ONUs, NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, "All", param['w'], param['p'])
            filename = "{}ONUs_{}OLT_{}km-{}-{}-r2score-{}_w{}-p{}".format(NUMBER_OF_ONUs, NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, "All", param['w'], param['p'])
            plt.title(title)
            plt.xlabel("load (%)")
            plt.ylabel("R2 Score")
            plt.yscale('linear')
            ...
            plt.clf()
            plt.figure(figsize=(14, 10))
            title = "{} ONUs - {} OLT ({}km) - Trafego {}, {} - média de MSE em 5s de simulação ({}) w{}-p{}".format(NUMBER_OF_ONUs, NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, "All",  param['w'], param['p'])
            filename = "{}ONUs_{}OLT_{}km-{}-{}-mse-{}_w{}-p{}".format(NUMBER_OF_ONUs, NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, "All", param['w'], param['p'])
            plt.title(title)
            plt.xlabel("load (%)")
            ...
            i=0
            for model in PRED_ALG:
                aux_df = pd.DataFrame(ipact_df_pr_msemedias[model])
                aux_label = "Ipact pred, {}, load(MB) = {:.3f}".format(model, float(PKT_SIZES[0])/(1024**2))
                media_pl = np.array(aux_df.iloc[:,0])
                aux_df = pd.DataFrame(ipact_df_pr_msestds[model])
