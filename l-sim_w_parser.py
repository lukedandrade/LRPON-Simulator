...
def openDirectories(payload_size, b_dir):
    try:
        os.makedirs('{}/{}km/{}/csv/delay'.format(b_dir, DISTANCE, payload_size))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    try:
        os.makedirs('{}/{}km/{}/csv/grant_time'.format(b_dir, DISTANCE, payload_size))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    try:
        os.makedirs('{}/{}km/{}/csv/grant_usage'.format(b_dir, DISTANCE, payload_size))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    try:
        os.makedirs("{}/{}km/{}/csv/pkt".format(b_dir, DISTANCE, payload_size))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    try:
        os.makedirs("{}/{}km/{}/csv/overlap".format(b_dir, DISTANCE, payload_size))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    try:
        os.makedirs("{}/{}km/general_information".format(b_dir, DISTANCE))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    try:
        os.makedirs("{}/{}km/{}/csv/metrics".format(b_dir, DISTANCE, payload_size))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise

#Recebe lista com delays em formato de dicionarios e obj de escrita
def delay_fileGeneration(delays_list, d_file, pkt_l_mean, pkt_l_std):
    df_delays = pd.DataFrame.from_dict(delays_list)
    media = df_delays['delay'].mean()
    std = df_delays['delay'].std()
    value_min = df_delays['delay'].min()
    value_max = df_delays['delay'].max()
    d_file.write("{},{},{},{},{},{}\n".format(media, std, value_min, value_max, pkt_l_mean, pkt_l_std))
...
class CBR_PG(PacketGenerator):
    ...
    def __init__(self,env, id, ONU, fix_pkt_size, interval=0.004):
        #interval é equivalente ao hold time
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            ...
            for i in range(npkt):
                ...
                p_list.append(p)
                #pkt_file.write("{},{},{},{}\n".format(self.ONU.oid,self.env.now, self.interval, self.fix_pkt_size))
...
class poisson_PG(PacketGenerator):
    ...
    def __init__(self,env, id, ONU, adist, sdist, fix_pkt_size):
        ...
        #1250 é o número de eventos ao longo de 5s, com base no holding time da CPRI1 (5/0.004)
        #Distribuição Poisson deveria determinar o número de eventos.
        #Outra distribuição é mais adequada para determinar o tempo entre eventos.
        if self.arrivals_dist != None:
            self.n_events = np.random.poisson(1250*self.arrivals_dist)
            self.t_between_evts = SIMULATION_TIME/self.n_events
        else:
            self.n_events = 1250
            self.t_between_evts = 0.004
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            # wait for next transmission
            if self.arrivals_dist != None:
                t_between_evts = np.random.exponential(self.t_between_evts)
            else:
            ...
            for i in range(npkt):
                ...
                if self.fix_pkt_size:
                    p = Packet(self.env.now, 1500, self.packets_sent, src=self.id)
                    p_list.append(p)
                    #pkt_file.write("{},{},{},{}\n".format(self.ONU.oid, self.env.now, t_between_evts, self.fix_pkt_size))
                else:
                    size = self.size_dist()
                    p = Packet(self.env.now, size, self.packets_sent, src=self.id)
                    p_list.append(p)
                    #pkt_file.write("{},{},{},{}\n".format(self.ONU.oid,self.env.now, t_between_evts, size))
            self.env.timeout(self.eth_overhead)
            for p in p_list:
                self.out.put(p) # put the packet in ONU port
...
class ONUPort(object):
    ...
    def __init__(self, env, ONU, qlimit=None):
        ...
        self.packet_loss = 0 #Taxa de pkt loss. pkt_drop/pkt_rec
        self.pkt_time_limit = 0 #tempo limite para drop de pacote em segundos, 0 indica sem tempo limite
    ...
    def send(self):
        ...
        while self.grant_final_time > self.env.now:
            ...
            if (self.grant_final_time <= self.env.now):
                ...
                break
            elif self.pkt is not None:
                pkt = self.pkt
                if ((self.env.now - pkt.time) > self.pkt_time_limit) and (self.pkt_time_limit != 0): #Testo o nível de delay do pacote, caso ele esteja acima de X ms é dropado
                    self.packets_drop += 1
                    self.packet_loss = self.packets_drop/self.packets_rec
                    pkt = None
                else:
                ...
                why_break = "no pkt"
                break
            if pkt is not None: #packet was not dropped bcs delay limit
                self.busy = 1
                ...
                delay_list.append({'onu_id': self.ONU.oid, 'grant_counter': Grant_ONU_counter[self.ONU.oid]-1, 'delay': (self.env.now - pkt.time)+self.ONU.delay, 'pkt_time': pkt.time, 'grant_type': grant_type})
                ...
                logging.debug("Packet created at {} lost because it was over the delay limit at {}.".format(self.pkt.time, self.env.now))
        ...
        if start_grant_usage and end_grant_usage > 0:# if any pkt has been sent
            ...
            #grant usage = [g_start, g_end, pkt_loss, data_sent, remaining_data]
            yield self.grant_real_usage.put([start_grant_usage , start_grant_usage + end_grant_usage, self.packet_loss, data_sent, self.byte_size])
        else:
            #print (why_break)
    ...
    def ONU_receiver(self,odn):
        while True:
            ...
            if len(grant_usage) == 0: #debug
                ...
                grant_usage_file.write("{},{},{},{},{},{},{},{}\n".format(self.oid, MAC_TABLE['olt'], MAC_TABLE[self.oid],False,
                                                              grant_usage[0], grant_usage[1], grant_usage[3], grant_usage[4]))
            ...
            if grant['prediction']:#check if have any predicion in the grant
                ...
                for pred in grant['prediction']:
                    ...
                    if len(grant_usage) > 0: # filling grant prediction report list
                        ...
                        #adicionar write de csv de grant usage
                        grant_usage_file.write("{},{},{},{},{},{},{},{}\n".format(self.oid, MAC_TABLE['olt'], MAC_TABLE[self.oid],True,
                                                              grant_usage[0], grant_usage[1], grant_usage[3], grant_usage[4]))
                    else:
    ...
    def ONU_sender(self, odn):
        ...
        while True:
            ...
            if self.port.byte_size >= self.bucket:
                ...
                #Delay de transmissão resolvido na classe ODN
...
class IPACT_Pred(DBA):
    def __init__(self, env, max_grant_size, grant_store, window=20, predict=5, model="ols"):
        ...
        for i in range(NUMBER_OF_ONUs):
            # training unit
            self.grant_history.insert(i, {'counter': [], 'start': [], 'end': [], 'pkt_loss': [], 'data_size': []})
        ...
        if model == "ols":
            ...
            reg = neural_network.MLPRegressor(hidden_layer_sizes=(2,), solver='lbfgs', activation='relu')
        elif model == 'd_tree':
    ...
    def predictor(self, ONU):
        ...
        #check if there's enough Grants to fill predictions' window
        if len(self.grant_history[ONU.oid]['start']) >= self.window:
            #pick up the last 'x' Grants to fill the window
            self.grant_history[ONU.oid]['start'] = self.grant_history[ONU.oid]['start'][-self.window:] #output
            self.grant_history[ONU.oid]['end'] = self.grant_history[ONU.oid]['end'][-self.window:] #output
            self.grant_history[ONU.oid]['counter'] = self.grant_history[ONU.oid]['counter'][-self.window:]
            self.grant_history[ONU.oid]['pkt_loss'] = self.grant_history[ONU.oid]['pkt_loss'][-self.window:]
            self.grant_history[ONU.oid]['data_size'] = self.grant_history[ONU.oid]['data_size'][-self.window:]

            #normalization of the high values of data_size
            self.grant_history[ONU.oid]['data_size'] = [x/1000000 for x in self.grant_history[ONU.oid]['data_size']]          

            df_tmp = pd.DataFrame(self.grant_history[ONU.oid]) #temporary dataframe
    
            aux = []
            for i in range(self.predict):
                aux.append([
                    self.grant_history[ONU.oid]['counter'][-1] + 1 + i,
                    self.grant_history[ONU.oid]['pkt_loss'][-1],
                    self.grant_history[ONU.oid]['data_size'][-1]
                    ])
            x_pred = np.array(aux).reshape(-1,3)

            #model fit
            inputs = np.array(df_tmp[['counter', 'pkt_loss', 'data_size']]).reshape(-1,3)
            outputs = df_tmp[['start','end']]
            X_train, X_test, Y_train, Y_test = train_test_split(inputs, outputs, test_size=0.2)
            self.model.fit(X_train, Y_train)
            test_predictions = self.model.predict(X_test)
            mse_score = mse(Y_test, test_predictions)
            r2_score = r2(Y_test, test_predictions)
            
            if (r2_score > 0) and (r2_score <= 1):
            ...
            if predictions is not None and (self.grant_history[ONU.oid]['end'][-1] + ONU.delay + self.guard_interval) > predictions[0][0]:
                #print("overlap between between standard grant and first prediction")
                #print(predictions)
                predictions = None
            
            #drop if there is overlap within predictions array
            if predictions is not None:
                if len(self.predictions_list) == 0:
                    self.predictions_list += predictions
                else:
                    self.predictions_list = list(filter(lambda x: x[0] > self.env.now, self.predictions_list))
                    predcp = list(predictions)
                    new_pred = []
                    drop = False
                    for interval_1 in predcp:
                        for interval_2 in self.predictions_list:
            ...
            if predictions != None:
                ai_metrics_file.write("{},{}\n".format(mse_score, r2_score))
            return predictions
        else:
            return None
        ...
        with self.counter.request() as my_turn:
            ...
            if len(ONU.grant_report) > 0:
                ...
                for report in ONU.grant_report:
                    ...
                    self.grant_history[ONU.oid]['pkt_loss'].append(report[2])
                    self.grant_history[ONU.oid]['data_size'].append(report[3])
            ...
            #checking the predictions_list
            if len(self.predictions_list) > 0:
                self.predictions_list = list(filter(lambda x: x[0] > self.env.now, self.predictions_list))

                if len(self.predictions_list) > 0:
                    #Checa se o tempo final de um grant real junto com tempos de transmissão invade o start da predição q está para acontecer
                    #Caso True altera o sending time com base no bucket limite da ONU.
                    #Versão Madson só alterava, acho que somar seria o correto.
                    if (grant_final_time + ONU.delay + self.guard_interval) > self.predictions_list[0][0]:
                        bits = ONU.bucket * 8
                        sending_time = bits/float(10000000000)
                        #grant_time = delay + sending_time #madson_version
                        grant_time += sending_time
                        grant_final_time = self.env.now + grant_time
            
            #Update grant history with requested grants
            self.grant_history[ONU.oid]['start'].append(self.env.now)
            self.grant_history[ONU.oid]['end'].append(grant_final_time)
            self.grant_history[ONU.oid]['pkt_loss'].append(ONU.port.packet_loss)
            if len (self.grant_history[ONU.oid]['counter']) > 0:
                self.grant_history[ONU.oid]['counter'].append(self.grant_history[ONU.oid]['counter'][-1] + 1)
            else:
                self.grant_history[ONU.oid]['counter'].append(1)
            self.grant_history[ONU.oid]['data_size'].append(buffer_size/1000000)
...
class OLT(object):
    ...
    def __init__(self,env,lamb,odn,max_grant_size,dba,window,predict,model,numberONUs):
        ...
        if dba == 'pd_dba':
            ...
            self.dba = IPACT_Pred(self.env, max_grant_size, self.grant_store, window, predict, model)
        else:
    ...
    for exp in EXPONENTS:
        ...
        for pkt_size in CPRI_PKT:
            for parameter in PARAMS:
                ...
                directory = "{}/{}km/{}/".format(BASE_DIR, DISTANCE, pkt_size)
                
                delay_file = open(directory+"csv/delay/{}-s{}-delay.csv".format(FILENAME, seed),"w")
                grant_time_file = open(directory+"csv/grant_time/{}-s{}-grant_time.csv".format(FILENAME, seed),"w")
                grant_usage_file = open(directory+"csv/grant_usage/{}-s{}-grant_usage.csv".format(FILENAME, seed),"w")
                #pkt_file = open(directory+"csv/pkt/{}-s{}-pkt.csv".format(FILENAME, seed),"w")
                ...
                ai_metrics_file.write("mse,r2_score\n")
                ...
                for i in range(NUMBER_OF_ONUs):
                    #Qlimit= n_pkts*1500, tá em bytes
                    ONU_list.append(
                        ONU(DISTANCE, i, env, lamb, channel, odn, pkt_size*0.95, BUCKET_SIZE, packet_generator, pg_params)
                    )
                ...
                pkt_std = 0
                for onu in ONU_list:
                    pkt_loss = onu.port.packet_loss
                    pkt_loss_list.append(pkt_loss)
                    aux_txt += "ONU {}-{} | ".format(onu.oid, pkt_loss)
                
                pkt_mean = np.mean(pkt_loss_list)
                pkt_std = np.std(pkt_loss_list)

                info_file.write("{},{},{},{}-{},{},{},{},{}\n".format(seed, exp, pkt_size, parameter['w'], parameter['p'],
                                                                 DBA_ALG, NUMBER_OF_ONUs, NUMBER_OF_OLTs,aux_txt))
                
                delay_fileGeneration(delay_list, delay_file, pkt_mean, pkt_std)
