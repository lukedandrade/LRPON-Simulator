...
for NUMBER_OF_ONUs in NUMBER_OF_ONUs_vet:
    for param in parameters:
        try:
            ...
            for model in PRED_ALG:
                for exp in exponents:
                    for size in PKT_SIZES:
                        ...
                        IPACT_Pred_r2_medio[size][exp] = media
                        IPACT_Pred_r2_std[size][exp] = std
            ...
            for model in PRED_ALG:
                for exp in exponents:
                    for size in PKT_SIZES:
                        ...
                        t_test_results = calculateT_student(ipact_mean, 10)
                        std = t_test_results['t-test amp']
                        IPACT_Pred_mse_medio[size][exp] = media
                        IPACT_Pred_mse_std[size][exp] = std
            ...
            title = "{} ONUs - {} OLT ({}km) - Trafego {}, {} - média de R2 Score em 5s de simulação ({}) w{}-p{}".format(NUMBER_OF_ONUs, NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, model, param['w'], param['p'])
            filename = "{}_ONUs_{}OLT_{}km-{}-{}-r2score-{}_w{}-p{}".format(NUMBER_OF_ONUs, NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, model, param['w'], param['p'])
            plt.title(title)
            plt.xlabel("load (%)")
            plt.ylabel("R2 Score")
            plt.yscale('linear')
            ...
            plt.clf()
            plt.figure(figsize=(14, 10))
            title = "{} ONUs - {} OLT ({}km) - Trafego {}, {} - média de MSE em 5s de simulação ({}) w{}-p{}".format(NUMBER_OF_ONUs, NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, model,  param['w'], param['p'])
            filename = "{}_ONUs_{}OLT_{}km-{}-{}-mse-{}_w{}-p{}".format(NUMBER_OF_ONUs, NUMBER_OF_OLTs, DISTANCE, TRAFFIC, DBA_ALG, model, param['w'], param['p'])
            plt.title(title)
            plt.xlabel("load (%)")
            ...
            i=0
            for size in PKT_SIZES:
                aux_df = pd.DataFrame(ipact_df_pr_msemedias[size])
                aux_label = "Ipact pred, {}, load(MB) = {:.3f}, n_ONUs = {}".format(model, float(size)/(1024**2), NUMBER_OF_ONUs)
                media_pl = np.array(aux_df.iloc[:,0])
                aux_df = pd.DataFrame(ipact_df_pr_msestds[size])
