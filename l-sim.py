...
if TRAFFIC == "poisson":
    ...
    EXPONENTS = [0.01, 0.05, 0.1, 0.20, 0.30, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.99] #0.1, 0.3, 0.5, 0.7, 0.9
    CPRI_PKT = [768000, 1536000, 3072000, 3840000] #, 1536000
    SEEDS = [20] #, 30, 40
else:
    EXPONENTS = [0]
...
class IPACT_Pred(DBA):
    ...
    def predictor(self, ONU):
        ...
        if len(self.grant_history[ONU.oid]['start']) >= self.window:
            ...
            #model fit
            self.model.fit(
                np.array(df_tmp[['counter', 'pkt_loss', 'data_size']]).reshape(-1,3),
                df_tmp[['start','end']]
            )
...
for seed in SEEDS:
    info_file = open("{}/csv/general_information/infos-s{}.csv".format(BASE_DIR, seed), "w")
    ...
    for exp in EXPONENTS:
        for pkt_size in CPRI_PKT:
            for parameter in PARAMS:
                ...
                #grant_time_prediction_file.write("source address,destination address,opcode,timestamp,counter,ONU_id,grant_size,start,end\n")
